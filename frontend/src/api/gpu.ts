import { api, uploadFile, streamTask } from './request'
import type {
...
export const gpuApi = {
  ...
  offlineInspect: (extractDir: string) => api<{ success: boolean; meta: Record<string, any> }>('/gpu/offline/inspect', { method: 'POST', body: { extract_dir: extractDir } }),
  ...
  uploadRunfile: (file: File) => uploadFile<{ success: boolean; path: string; filename: string; size: number; message?: string }>('/upload/runfile', file),
  uploadOffline: (file: File) => uploadFile<{ success: boolean; extract_dir?: string; path?: string; name?: string; meta?: Record<string, any>; package_count?: number; has_install_script?: boolean; size?: number; message?: string }>('/upload/offline', file),
}
